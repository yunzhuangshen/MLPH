...
#include "my_tclique/tclique.h"
#include "../pricing/init_random.h"
...
SCIP_DECL_HEUREXEC(heurExecInit)
{
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   if( COLORprobGetNStableSets(scip) == 0 )
   {
      ...
      primal_heur_time = get_wall_time();
      if( heurdata->usetabu )
      {
      ...
      primal_heur_time = get_wall_time() - primal_heur_time;
   ...
   // add additionally 10n - ncolor random solutions
   // mt19937 mt(heurdata->seed);
   // uniform_int_distribution<int> dist(0,RAND_MAX);
   // int v, idx, num;
   // int nb_node =  tcliqueGetNNodes(graph);
   // vector<vector<bool>> adj_matrix(nb_node, vector<bool>(nb_node, false));
   ...
   // int nb_candidates;
   // vector<int> col;
   // for(i = 0; i < 10 * nb_node - ncolors; ++i) {
   //    col.clear();
   ...
   )
{
   ...
   SCIP_CALL( SCIPaddIntParam(scip,
      "heuristics/initcol/seed",
      "random seed for initializing columns",
      &heurdata->seed, TRUE, seed, 0, INT_MAX, NULL, NULL) );
