...
void FASTWCLQ::build(string file_name) {
...
void FASTWCLQ::print_vec(vector<long> & container) {
	cout << '{';
...
long FASTWCLQ::upper_bound(long v) {
	if (adjacency_list[v].empty()) {
		return vertex_weight[v];
	}
	long largest_weight_neighbor = 0;
	long largest_weight = -1;
	...
	long v1, v2;
	// cout << adjacency_list.size() << " " << v << " " << largest_weight_neighbor << " " << adjacency_list[v].size() << " " << vertex_weight[adjacency_list[v][0]] << "\n";
...
void FASTWCLQ::simplify_iterative() {
...
template<typename T>
bool FASTWCLQ::is_neighbor(T v1, T v2) {//check v1 in adj list of v2
...
void FASTWCLQ::output_graph_size() {
	long edge_count = 0;
	for (auto v : remaining_vertex) {
...
void FASTWCLQ::output_best_solution() {
...
void FASTWCLQ::update_best_solution() {
	best_solution.clear();
	for (auto v : solution) {
...
bool FASTWCLQ::compare_vertex (long v1, long v2) {
...
vector<long>::size_type FASTWCLQ::binary_search(const vector<long> &array, long value) {
...
bool FASTWCLQ::verify_simple(string file_name) {
	cout << "verifying..." << endl;
	ifstream in_file(file_name);
	if (! in_file.is_open()) {
		cout << "in_file error" << endl;
...
bool FASTWCLQ::verify (string file_name) 
{
	cout << "verifying..." << endl;
...
long long FASTWCLQ::fastwclq(long ntries, long seed, long long vertex_count, long long nb_edge, double cutoff_time, long long** AdjacentList, long long* Node_Degree, long long* Node_Weight, long long* Node_Bound){
...
//	build(filename);
	...
	for (long long i = 0; i < vertex_count; i++){
	    for (long long j = 0; j < Node_Degree[i]; j++){
	...
	for(tries=1; tries<=ntries; tries++) {
	
		if (tries%vertex_count==0) {
			srand(seed++); 
		}
		...
		sol_objs.push_back(solution_weight);
		sol_times.push_back((double) ( clock() - start) / CLOCKS_PER_SEC);
		vector<int> sol;
		for (auto v : solution)
			sol.push_back(v-1);
		sols.push_back(sol);
