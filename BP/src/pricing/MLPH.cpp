...
namespace GCP {
    ...
    MLPH::MLPH(bool is_root, METHOD_TYPE _variant, double _cutoff, MWISP_INST& inst, double sample_factor): 
        degree_norm{inst.degree_norm}, adj_matrix{inst.adj_matrix}{
            
        dual_values = inst.dual_values;
        variant = _variant;
        assert(variant == METHOD_TYPE::BP_MLPH || 
                variant == METHOD_TYPE::BP_MLPH_FORCE_EXACT);
        nb_node = inst.nb_node;
        nb_edge = inst.nb_edge;
        cutoff = _cutoff;
        random_sample_size = inst.nb_node;
        sample_size = ceil(inst.nb_node * sample_factor);
    ...
    void MLPH::random_sampling() {
        objs = std::vector<double> (random_sample_size);
        mis_set = std::vector<std::vector<int>> (random_sample_size);
    ...
    void MLPH::compute_ranking_based_measure() {
        ...
        std::vector<int> sort_idx(random_sample_size);
        ...
        std::vector<int> rank = std::vector<int>(random_sample_size);
        for (auto i = 0; i < random_sample_size; i++){
            rank[sort_idx[i]] = i;
        }
    ...
    void MLPH::compute_correlation_based_measure(){
        ...
        mean_y = mean_y/random_sample_size;
        std::vector<float> diff_y = std::vector<float>(random_sample_size);
        ...
        for (auto i = 0; i < random_sample_size; ++i){
            for (auto j = 0; j < mis_set[i].size(); ++j){
                mean_x[mis_set[i][j]] += 1.0/random_sample_size;
        ...
        for (auto i = 0; i < nb_node; ++i){
            variance_x[i] = mean_x[i]*(1-mean_x[i])*random_sample_size;
