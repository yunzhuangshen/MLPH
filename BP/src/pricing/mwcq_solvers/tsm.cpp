...
#include "tsm.hpp"
#include <cassert>
namespace TSM_SOLVER {



double TSM::get_utime() {
	struct rusage utime;
...
int TSM::is_adjacent(int node1, int node2) {
	int neibor, *neibors;
...
void TSM::check_clique_in_result_file(char *input_file) {
...
void TSM::allcoate_memory_for_adjacency_list(int nb_node, int nb_edge, int offset) {
...
int TSM::read_graph_wclq_format(char *input_file) {
...
int TSM::read_graph_node_node(char *input_file, int format) {
	int j, node = 1, l_node, r_node, nb_edge = 0, max_node = NONE, offset = 0;
	char line[128], word[10];
	FILE* fp_in = fopen(input_file, "r");
	if (fp_in == NULL ) {
...
int TSM::build_simple_graph_instance(char *input_file) {
...
void TSM::sort_by_degree_degeneracy_ordering() {
...
void TSM::sort_by_score_ordering() {

//	INIT_CLQ_SIZE = 0;
//	INIT_CLQ_WEIGHT = 0;


	long long Max_Weight, Max_Degree;
//	printf("I computing vertex degeneracy ordering...\n");
//
//	printf("the number of node is %d\n", NB_NODE);
...
long long TSM::init_for_degeneracy(long long *segment_counter, long long *where) {
	long long i, j, t, total_weight = 0;
...
long long TSM::sort_by_weight_degeneracy_and_core_decomposion() {
	int *neibors, neibor;
	long long new_weight, total_weight = 0, pre_weight, cur_weight;
...
int TSM::addIntoIsetTomitaBis_adj(int node) {
...
int TSM::re_number_adj(int node) {
...
long long TSM::absorb_by_inserting(int iset_idx, long long iset_weight, int insert_node, long long node_weight, int last_iset) {
...
long long TSM::absorb_by_splitting(int iset_idx, long long topk_weight, int insert_node, long long node_weight) {
	int k, iset_node;
...
void TSM::do_weight_partition(int insert_node, long long node_weight) {
...
int TSM::add_vertex_with_weight_partition(int insert_node) {
...
//void check_build_structures();

int TSM::cut_by_iset_less_vertices() {
...
int TSM::cut_by_binary_maxsat_reasoning() {
...
int TSM::add_new_iset_for_bnode(int b_node, long long b_weight) {
...
void TSM::init_for_maxsat_reasoning() {
	int i;
	ptr(UNIT_STACK) = 0;
	ptr(REASON_STACK) = 0;
...
//void release_conflicting_isets() {
...
//int retrace_conflicting_isets2(int start_iset) {
...
int TSM::retrace_conflicting_isets(int start_iset) {
	int i, iset, reason, node;
	long long *nodes;
	int start, count = 0;
...
int TSM::fix_node(int fix_node, int fix_iset) {
	int i, j, iset, node;
...
void TSM::reduce_iset(int iset_idx, long long delta) {
	int size = 0, topk = 0;
	int node;
	long long *nodes;
...
void TSM::reduce_iset_topk_new(int iset_idx, long long delta, long long t_weight) {
	int topk = 0;
	int node;
...
void TSM::split_conflicting_isets() {
...
long long TSM::insert_into_iset(int iset_idx, long long iset_weight, int insert_node, long long node_weight) {
	long long insert_weight, *iset_addr, *_addr;
...
long long TSM::split_and_insert(int iset_idx, long long t_weight, int insert_node, long long node_weight) {
	int node;
...
int TSM::insert_vertex_with_max3sat(int insert_node) {
	int iset_idx, node;
...
int TSM::cut_by_ordered_maxsat_reasoning() {
	int i, k, node;
...
void TSM::build_iset_structures() {
	int i, node, idx;
	long long *nodes;
	int iset_len = 0;
...
void TSM::rebuild_matrix(int start) {
...
void TSM::store_maximal_weighted_clique() {
...
long long TSM::compute_subgraphs_weight(int start) {
...
void TSM::store_maximal_weighted_clique2() {
	memcpy(MaxCLQ_Stack, Clique_Stack, CUR_CLQ_SIZE * sizeof(int));
...
long long TSM::init_for_me(int start, long long *segment_counter, long long *where) {
...
int TSM::compute_subgraph_degree(int start) {
...
int TSM::reduce_first_level_subgraphs_by_degree(int start) {
...
int TSM::reduce_first_level_subgraphs_by_weight(int start) {
...
int TSM::cut_by_inc_ub() {
	...
	if((max+CUR_NODE_WEIGHT+CUR_CLQ_WEIGHT<=MAX_CLQ_WEIGHT)||(CUR_CLQ_WEIGHT+max_weight<=MAX_CLQ_WEIGHT)) { //
			...
			if(CUR_CLQ_SIZE==0) {
				...
				if(REBUILD_MATRIX==TRUE || CUR_MAX_NODE > MATRIX_SIZE) {
...
void TSM::search_maxclique(double cutoff, int using_init_clique) {
	int node;
...
void TSM::printMaxClique(char* input_file) {
	printf("M ");
//	ifstream FIC;
	FILE *fp ;
...
void TSM::reduce_instance_for_weight() {
	...
	free_block();
	BLOCK_COUNT = 1;
	BLOCK_LIST[BLOCK_COUNT - 1] = Adj_List;
...
void TSM::reduce_instance_for_degree() {
	int i, j, nb, node, *neibors, *neibors2, *addr;
	MAX_SUBGRAPH_SIZE = 0;
	N0_B = NB_NODE;
	j = 0;
...
void TSM::check_result(char *input_file, char *result_file) {
	char * fileStyle = strrchr(input_file, '.') + 1;
...
void TSM::clear_structures(){
	memset(Vertex_UB,0,MAX_NODE*2*sizeof(long long));
}

TSM::TSM(){
	free_block();
	Node_Degree = (long long*)malloc(MAX_NODE*sizeof(long long));
	Top_Weight = (long long*)malloc(MAX_NODE*sizeof(long long));
	Node_Weight = (long long*)malloc(MAX_NODE*sizeof(long long));
	Node_Value = (char*)malloc(MAX_NODE*sizeof(char));
	rankVar= (int*)malloc(MAX_NODE*sizeof(int));
	Candidate_Stack = (long long*)malloc(MAX_NODE*2*sizeof(long long));
	Vertex_UB = (long long*)malloc(MAX_NODE*2*sizeof(long long));
}
...
TSM::~TSM(){
	...
	free(Node_Value);
	free(rankVar);
	free(Candidate_Stack);
	free(Vertex_UB);
}

long long TSM::tsm(char* File_Name1, const char* resPath1, int NB_NODE1, int NB_EDGE1, double cutoff, long long** AdjacentList, long long* Node_Degree1, long long* Node_Weight1, long long* Node_Bound1){
